...
def create_tdc_dataset(df, fold, smile_graph, seed):
    ## params
    max_seqlen = 1000
    ...
    ## dataset
    return TestbedDataset(root='data', dataset=f'tdc_{fold}_{seed}', xd=xd, xt=xt, y=y, smile_graph=smile_graph, use_tqdm=True)
...
def main():
    ...
    ## Data split
    frames = data.get_split(method='cold_drug', seed=random_seed, frac=[0.8, 0.1, 0.1])
    df_tr = frames['train']
    df_va = frames['valid']
    df_te = frames['test']
    ...
    train_data = create_tdc_dataset(df_tr, 'train', smile_graph, random_seed)
    test_data = create_tdc_dataset(df_te, 'test', smile_graph, random_seed)
    valid_data = create_tdc_dataset(df_va, 'valid', smile_graph, random_seed)
    ...
    trainer.train(train_data, test_data, valid_data,
                  ckpt_dir='./ckpt', ckpt_filename=f'{modeling}_tdc_{random_seed}')
